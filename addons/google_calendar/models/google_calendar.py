...
from eagle.tools import exception_to_unicode
...
class GoogleCalendar(models.AbstractModel):
    ...
    def update_events(self, lastSync=False):
        ...
        if lastSync:
            ...
            my_eagle_attendees = CalendarAttendee.with_context(context_novirtual).search([
                ('partner_id', '=', my_partner_id),
                ('event_id.oe_update_date', '>', lastSync and fields.Datetime.to_string(lastSync) or self.get_minTime().fields.Datetime.to_string()),
                ('google_internal_event_id', '!=', False),
            ])

            my_eagle_googleinternal_records = my_eagle_attendees.read(['google_internal_event_id', 'event_id'])

            if self.get_print_log():
                _logger.info("Calendar Synchro -  \n\nUPDATE IN GOOGLE\n%s\n\nRETRIEVE FROM OE\n%s\n\nUPDATE IN OE\n%s\n\nRETRIEVE FROM GG\n%s\n\n", all_event_from_google, my_google_att_ids, my_eagle_attendees.ids, my_eagle_googleinternal_records)

            for gi_record in my_eagle_googleinternal_records:
            ...
            my_attendees = (my_google_attendees | my_eagle_attendees)
