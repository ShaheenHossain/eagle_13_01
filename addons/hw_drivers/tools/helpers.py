...
def add_credential(db_uuid, enterprise_code):
    write_file('eagle-db-uuid.conf', db_uuid)
    write_file('eagle-enterprise-code.conf', enterprise_code)
    ...
    if server:
        ...
        if path.exists():
            with path.open('r') as f:
                ...
                if cn == 'EagleTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected Eagle DB and
    ...
    if server:
        ...
        try:
            ...
            if response.status == 200:
                git = ['git', '--work-tree=/home/pi/eagle/', '--git-dir=/home/pi/eagle/.git']
                ...
                if db_branch != local_branch:
                    ...
                    os.system('/home/pi/eagle/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh')
...
def get_eagle_server_url():
    return read_file_first_line('eagle-remote-server.conf')
...
def load_certificate():
    """
    Send a request to Eagle with customer db_uuid and enterprise_code to get a true certificate
    """
    db_uuid = read_file_first_line('eagle-db-uuid.conf')
    enterprise_code = read_file_first_line('eagle-enterprise-code.conf')
    if db_uuid and enterprise_code:
        url = 'https://www.eagle.com/eagle-enterprise/iot/x509'
        data = {
            'params': {
                'db_uuid': db_uuid,
        ...
        if result:
            write_file('eagle-subject.conf', result['subject_cn'])
            subprocess.check_call(["sudo", "mount", "-o", "remount,rw", "/"])
...
def download_drivers(auto=True):
    """
    Get the drivers from the configured Eagle server
    """
    server = get_eagle_server_url()
    if server:
        urllib3.disable_warnings()
