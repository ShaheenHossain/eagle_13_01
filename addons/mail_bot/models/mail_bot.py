...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, record, values, command=None):
        ...
        The logic will only be applied if eaglebot is in a chat with a user or
        if someone pinged eaglebot.

         :param record: the mail_thread (or mail_channel) where the user
            message was posted/eaglebot will answer.
        ...
        if len(record) != 1 or values.get("author_id") == eaglebot_id:
            return
        if self._is_bot_pinged(values) or self._is_bot_in_private_channel(record):
            ...
            if answer:
                ...
                record.with_context(mail_create_nosubscribe=True).sudo().message_post(body=answer, author_id=eaglebot_id, message_type=message_type, subtype_id=subtype_id)
    ...
    def _get_answer(self, record, body, values, command=False):
        ...
        if self._is_bot_in_private_channel(record):
            # main flow
            if eaglebot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.eaglebot_state = "onboarding_attachement"
                return _("Great! 👍<br/>Now, try to <b>send an attachment</b>, like a picture of your cute dog...")
            elif eaglebot_state == 'onboarding_attachement' and values.get("attachment_ids"):
                self.env.user.eaglebot_state = "onboarding_command"
                return _("Not a cute dog, but you get it 😊<br/>To access special features, <b>start your sentence with '/'</b>. Try to get help.")
            elif eaglebot_state == 'onboarding_command' and command == 'help':
                self.env.user.eaglebot_state = "onboarding_ping"
                return _("Wow you are a natural!<br/>Ping someone to grab its attention with @nameoftheuser. <b>Try to ping me using @EagleBot</b> in a sentence.")
            elif eaglebot_state == 'onboarding_ping' and self._is_bot_pinged(values):
                self.env.user.eaglebot_state = "idle"
                return _("Yep, I am here! 🎉 <br/>You finished the tour, you can <b>close this chat window</b>. Enjoy discovering Eagle.")
            elif eaglebot_state == "idle" and (_('start the tour') in body.lower()):
                self.env.user.eaglebot_state = "onboarding_emoji"
            ...
            return random.choice([_("Yep, EagleBot is in the place!"), _("Pong.")])
        return False
    ...
    def _is_bot_pinged(self, values):
        eaglebot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        return eaglebot_id in values.get('partner_ids', [])

    def _is_bot_in_private_channel(self, record):
        eaglebot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        if record._name == 'mail.channel' and record.channel_type == 'chat':
            return eaglebot_id in record.with_context(active_test=False).channel_partner_ids.ids
        return False
