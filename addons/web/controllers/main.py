...
import eagle
import eagle.modules.registry
from eagle.api import call_kw, Environment
from eagle.modules import get_module_path, get_resource_path
from eagle.tools import image_process, topological_sort, html_escape, pycompat, ustr, apply_inheritance_specs, lazy_property
from eagle.tools.mimetypes import guess_mimetype
from eagle.tools.translate import _
from eagle.tools.misc import str2bool, xlsxwriter, file_open
from eagle.tools.safe_eval import safe_eval
from eagle import http, tools
from eagle.http import content_disposition, dispatch_rpc, request, serialize_exception as _serialize_exception, Response
from eagle.exceptions import AccessError, UserError, AccessDenied
from eagle.models import check_method_name
from eagle.service import db, security
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('eagle.addons.web', "views")
    ...
    try:
        ...
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, eagle.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = eagle.conf.server_wide_modules or []
    serverside = ['base', 'web']
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("eagle.addons.web.main.manifest_list(): debug parameter is deprecated")
    mods = mods.split(',')
...
def fix_view_modes(action):
    """ For historical reasons, Eagle has weird dealings in relation to
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.httprequest.method == 'POST':
            ...
            except eagle.exceptions.AccessDenied as e:
                request.uid = old_uid
                if e.args == eagle.exceptions.AccessDenied().args:
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = eagle.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def version_info(self):
        return eagle.service.common.exp_version()
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = eagle.tools.config.verify_admin_password('admin')
        d['list_db'] = eagle.tools.config['list_db']
        d['langs'] = eagle.service.db.exp_list_lang()
        d['countries'] = eagle.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = eagle.service.db.list_db_incompatible(d['databases'])
        except eagle.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        try:
            eagle.service.db.check_super(master_pwd)
            ...
            dump_stream = eagle.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=eagle.SUPERUSER_ID))
    ...
    def account(self):
        ...
        return 'https://accounts.eagle.com/oauth2/auth?' + werkzeug.url_encode(params)
    ...
    def _content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        if not image_base64:
            ...
            if not (width or height):
                width, height = eagle.tools.image_guess_size_from_field_name(field)
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = eagle.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: eagle.tools.ustr(field[1].get('string', '').lower()))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from Eagle's export data to whatever the
