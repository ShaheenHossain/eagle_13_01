...
``views``
    ...
    :meth:`~eagle.models.Model.fields_view_get`). The first type of the
...
.. autoclass:: eagle.addons.base.models.ir_actions.IrActionsServer
...
``model_id``
    Eagle model linked to the action.
...
``fields_lines`` (create/write)
    ...
    :class:`~eagle.fields.One2many` with the fields:
...
``groups_id``
    :class:`~eagle.fields.Many2many` field to the groups allowed to view/use
    the current report
``multi``
    if set to ``True``, the action will not be displayed on a form view.
``paperformat_id``
    :class:`~eagle.fields.Many2one` field to the paper format you wish to
