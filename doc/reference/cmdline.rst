...
================================
Command-line interface: eagle-bin
================================
...
.. option:: -s, --save
    ...
    (:file:`{$HOME}/.eaglerc` by default, and can be overridden using
    :option:`-c`).
...
.. option:: --screenshots
    ...
    fails. It defaults to :file:`/tmp/eagle_tests/{db_name}/screenshots`
...
.. option:: --db-filter <filter>
    ...
    - ``%d`` is replaced by the subdomain the request is made on, with the
      exception of ``www`` (so domain ``eagle.com`` and ``www.eagle.com`` both
      match the database ``eagle``).

      These operations are case sensitive. Add option ``(?i)`` to match all
      databases (so domain ``eagle.com`` using ``(?i)%d`` matches the database
      ``Eagle``).
    ...
    .. code-block:: bash
        ...
        $ eagle-bin --db-filter ^11.*$

    Restrict access to databases whose name starts with 11

    .. code-block:: bash

        $ eagle-bin --database 11firstdatabase,11seconddatabase

    Restrict access to only two databases, 11firstdatabase and 11seconddatabase

    .. code-block:: bash

        $ eagle-bin --database 11firstdatabase,11seconddatabase -u base
    ...
    .. code-block:: bash
        ...
        $ eagle-bin --db-filter ^11.*$ --database 11firstdatabase,11seconddatabase -u base
...
.. option:: --smtp-ssl
    ...
    If set, eagle should use SSL/STARTSSL SMTP connections
...
.. option:: --log-handler <handler-spec>
    ...
    e.g. ``eagle.models:DEBUG`` will enable all logging messages at or above
    ...
    * If the level is omitted, the logger is set to ``INFO``
    ...
    .. code-block:: console
        ...
        $ eagle-bin --log-handler :DEBUG --log-handler werkzeug:CRITICAL --log-handler eagle.fields:WARNING

.. option:: --log-request

    enable DEBUG logging for RPC requests, equivalent to
    ``--log-handler=eagle.http.rpc.request:DEBUG``

.. option:: --log-response

    enable DEBUG logging for RPC responses, equivalent to
    ``--log-handler=eagle.http.rpc.response:DEBUG``

.. option:: --log-web

    enables DEBUG logging of HTTP requests and responses, equivalent to
    ``--log-handler=eagle.http:DEBUG``

.. option:: --log-sql

    enables DEBUG logging of SQL querying, equivalent to
    ``--log-handler=eagle.sql_db:DEBUG``
...
.. option:: --log-level <level>
    ...
    ``debug_rpc``
        sets the ``eagle`` and HTTP request loggers to ``debug``

        equivalent to ``--log-level debug --log-request``
    ``debug_rpc_answer``
        sets the ``eagle`` and HTTP request and response loggers to
        ``debug``
...
The default configuration file is :file:`{$HOME}/.eaglerc` which
can be overridden using :option:`--config <eagle-bin -c>`. Specifying
:option:`--save <eagle-bin -s>` will save the current configuration state back
to that file.
...
Eagle command-line also allows to launch eagle as a python console environment.
...
Scaffolding is available via the :command:`eagle-bin scaffold` subcommand.
...
.. code-block:: console
    ...
    $ eagle_bin scaffold my_module /addons/
