...
.. note::
  Setting a proper :option:`--db-filter <eagle-bin --db-filter>` is an important part
...
create a new user (``eagle``) and set it as the database user.
...
* Multiprocessing is enabled by configuring :option:`a non-zero number of
  worker processes <eagle-bin --workers>`, the number of workers should be based
...
listening on :option:`the longpolling port <eagle-bin --longpolling-port>` but
...
in ``/etc/nginx/sites-enabled/eagle.conf`` set:
...
.. code-block:: nginx
  ...
  server {
     listen 80;
     server_name eagle.mycompany.com;
  ...
  server {
   ...
   # log
   access_log /var/log/nginx/eagle.access.log;
   error_log /var/log/nginx/eagle.error.log;

   # Redirect longpoll requests to eagle longpolling port
   location /longpolling {
   proxy_pass http://eaglechat;
   }

   # Redirect requests to eagle backend server
   location / {
     proxy_redirect off;
...
* Which should not be exposed to the network. To ensure cron runners are not
  ...
  entirely with :option:`eagle-bin --no-http` or setting ``http_enable = False``
...
- Use appropriate database filters ( :option:`--db-filter <eagle-bin --db-filter>`)
  to restrict the visibility of your databases according to the hostname.
  See :ref:`db_filter`.
  You may also use :option:`-d <eagle-bin -d>` to provide your own (comma-separated)
...
- Once your ``db_name`` and ``db_filter`` are configured and only match a single database
  ...
  (this is also exposed as the :option:`--no-database-list <eagle-bin --no-database-list>`
  command-line option)

- Make sure the PostgreSQL user (:option:`--db_user <eagle-bin --db_user>`) is *not* a super-user,
  ...
  See also :ref:`setup/deploy/eagle`.
...
- Keep installations updated by regularly installing the latest builds,
  ...
  https://www.eagle.com/page/download or http://nightly.eagle.com
...
- Run Eagle behind a web server providing HTTPS termination with a valid SSL certificate,
  ...
  and then enable the :option:`proxy mode <eagle-bin --proxy-mode>` option.
...
Failed login::
      ...
      2018-07-05 14:56:31,506 24849 INFO db_name eagle.addons.base.res.res_users: Login failed for db:db_name login:admin from 127.0.0.1

Successful login::

      2018-07-05 14:56:31,506 24849 INFO db_name eagle.addons.base.res.res_users: Login successful for db:db_name login:admin from 127.0.0.1
...
10 failed login attempts are detected from the same IP within 1 minute::
    ...
    logpath = /var/log/eagle.log  ;  set the actual eagle log path here
...
:ref:`setup/deploy/eagle` mentioned ``admin_passwd`` in passing.
...
up-to-date. Eagle is supported on the current browser version. We support
...
.. note::
    ...
    Since Eagle 13.0, ES6 is supported.  Therefore, IE support is dropped.
...
.. _POSBox: https://www.eagle.com/page/point-of-sale-hardware#part_2
