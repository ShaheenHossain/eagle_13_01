...
.. only:: html
    ...
    .. rst-class:: setupcode hidden
        ...
        .. code-block:: python3
            ...
            info = xmlrpc.client.ServerProxy('https://demo.eagle.com/start').start()
        ...
        .. code-block:: ruby
            ...
            info = XMLRPC::Client.new2('https://demo.eagle.com/start').call('start')
            url, db, username, password = \
                info['host'], info['database'], info['user'], info['password']
        ...
        .. code-block:: php
            ...
            $info = ripcord::client('https://demo.eagle.com/start')->start();
        ...
        .. code-block:: java
            ...
            start_config.setServerURL(new URL("https://demo.eagle.com/start"));
...
.. warning::
    ...
    For Eagle Online instances (<domain>.eagle.com), users are created without a
    ...
    *https://mycompany.eagle.com*), the *database name* is the name of the
...
To make exploration simpler, you can also ask https://demo.eagle.com for a test
database:
...
The second endpoint is ``xmlrpc/2/object``, is used to call methods of eagle
...
Records can be listed and filtered via :meth:`~eagle.models.Model.search`.

:meth:`~eagle.models.Model.search` takes a mandatory
...
:meth:`~eagle.models.Model.search_count` can be used to retrieve
only the number of records matching the query. It takes the same
:ref:`domain <reference/orm/domains>` filter as
:meth:`~eagle.models.Model.search` and no other parameter.
...
Record data is accessible via the :meth:`~eagle.models.Model.read` method,
which takes a list of ids (as returned by
:meth:`~eagle.models.Model.search`) and optionally a list of fields to
...
:meth:`~eagle.models.Model.fields_get` can be used to inspect
...
Its arguments are similar to :meth:`~eagle.models.Model.search`'s, but it
can also take a list of ``fields`` (like :meth:`~eagle.models.Model.read`,
...
Records of a model are created using :meth:`~eagle.models.Model.create`. The
method will create a single record and return its database identifier.

:meth:`~eagle.models.Model.create` takes a mapping of fields to values, used
...
.. warning::
    ...
    :class:`~eagle.fields.Integer`, string for :class:`~eagle.fields.Char`
    or :class:`~eagle.fields.Text`),

    * :class:`~eagle.fields.Date`, :class:`~eagle.fields.Datetime` and
      :class:`~eagle.fields.Binary` fields use string values
    * :class:`~eagle.fields.One2many` and :class:`~eagle.fields.Many2many`
      use a special command protocol detailed in :meth:`the documentation to
      the write method <eagle.models.Model.write>`.

Update records
--------------

Records can be updated using :meth:`~eagle.models.Model.write`, it takes
a list of records to update and a mapping of updated fields to values similar
to :meth:`~eagle.models.Model.create`.
...
:meth:`~eagle.models.Model.unlink`.
...
``field_id``
    list of the model's fields through a :class:`~eagle.fields.One2many` to
    :ref:`reference/webservice/inspection/fields`
``view_ids``
    :class:`~eagle.fields.One2many` to the :ref:`reference/views` defined
    for the model
``access_ids``
    :class:`~eagle.fields.One2many` relation to the
...
``model_id``
    :class:`~eagle.fields.Many2one` to
...
``groups``
    ...
    :class:`~eagle.fields.Many2many` to ``res.groups``
