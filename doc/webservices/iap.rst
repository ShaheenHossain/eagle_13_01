...
.. note:: The credits went from integer to float value starting **October 2018**.
    ...
    * for a postage service: 1 credit = 1 post stamp.
    ...
    that the clients can buy on https://iap.eagle.com (see :ref:`Packs <iap-packages>`).
...
go to your *Portal Account* on the IAP endpoint (https://iap.eagle.com for
production, https://iap-sandbox.eagle.com for testing, the endpoints are
*independent* and *not synchronized*). Alternatively, you can go to your portal
on Eagle (https://iap.eagle.com/my/home) and select *In-App Services*.
...
First, we will create an *eagle module* depending on ``iap``. IAP is a standard
...
.. note::
    ...
    :class:`~eagle.addons.iap.models.iap.InsufficientCreditError` coming from the action
    and prompts the user to add credits to their account.

    :func:`~eagle.addons.iap.jsonrpc` takes care of re-raising
    :class:`~eagle.addons.iap.models.iap.InsufficientCreditError` for you.

.. danger::

    If you are not using :func:`~eagle.addons.iap.jsonrpc` you *must* be
    careful to re-raise
    :class:`~eagle.addons.iap.models.iap.InsufficientCreditError` in your handler
...
corresponding controller which can call :class:`~eagle.addons.iap.models.iap.charge` and
perform the service within:

.. patch::

.. todo:: for the actual IAP will the "portal" page be on eagle.com or iap.eagle.com?

.. todo:: "My Account" > "Your InApp Services"?


The :class:`~eagle.addons.iap.models.iap.charge` helper will:
...
.. danger::
    ...
    By default, :class:`~eagle.addons.iap.models.iap.charge` contacts the *production*
    IAP endpoint, https://iap.eagle.com. While developing and testing your
    service you may want to point it towards the *development* IAP endpoint
    https://iap-sandbox.eagle.com.
...
The :class:`~eagle.addons.iap.models.iap.charge` helper has two additional optional
...
.. function:: /iap/1/authorize
    ...
    *and creates a hold (pending transaction) on that amount*.
    ...
    :raises: :class:`~eagle.exceptions.AccessError` if the service token is invalid
    :raises: :class:`~eagle.addons.iap.models.iap.InsufficientCreditError` if the account does not have enough credits
...
.. function:: /iap/1/capture
    ...
    :raises: :class:`~eagle.exceptions.AccessError`
...
.. class:: ServiceName
    ...
    String identifying your service on https://iap.eagle.com (production) as well
...
.. exception:: eagle.addons.iap.models.iap.InsufficientCreditError
...
.. exception:: eagle.exceptions.AccessError
    :noindex:

    Raised by:

    * any operation to which a service token is required, if the service token is invalid; or
    * any failure in an inter-server call. (typically, in :func:`~eagle.addons.iap.jsonrpc`).

.. exception:: eagle.exceptions.UserError
    :noindex:
...
* Token ``000000``: Represents a non-existing account. Returns
  an :class:`~eagle.addons.iap.models.iap.InsufficientCreditError` on authorize attempt.
* Token ``000111``: Represents an account without sufficient credits to perform any service.
  Returns an :class:`~eagle.addons.iap.models.iap.InsufficientCreditError` on authorize attempt.
...
.. class:: eagle.addons.iap.models.iap.charge(env, key, account_token, credit[, dbuuid, description, credit_template])
    ...
    * otherwise cancels it.
    ...
    :param eagle.api.Environment env: used to retrieve the ``iap.endpoint``
                                     configuration key
    :param ServiceKey key:
...
.. class:: eagle.addons.iap.models.iap.authorize(env, key, account_token, credit[, dbuuid, description, credit_template])
...
.. class:: eagle.addons.iap.models.iap.cancel(env, transaction_token, key)
...
.. class:: eagle.addons.iap.models.iap.capture(env, transaction_token, key, credit)
...
.. _Eagle App: https://www.eagle.com/apps
