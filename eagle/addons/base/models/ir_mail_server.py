...
from eagle.exceptions import except_orm, UserError
from eagle.tools import ustr, pycompat

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('eagle.tests')
...
class IrMailServer(models.Model):
    ...
    def test_smtp_connection(self):
        for server in self:
            ...
            try:
                ...
                email_from, email_to = self.env.user.email, 'noreply@eagle.com'
                if not email_from:
    ...
    def _get_default_bounce_address(self):
        ...
        If "mail.bounce.alias" is not set it defaults to "postmaster-eagle".

        If "mail.catchall.domain" is not set, return None.

        '''
        get_param = self.env['ir.config_parameter'].sudo().get_param
        postmaster = get_param('mail.bounce.alias', default='postmaster-eagle')
