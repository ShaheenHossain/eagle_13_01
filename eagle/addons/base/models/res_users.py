...
from eagle import api, fields, models, tools, SUPERUSER_ID, _
from eagle.exceptions import AccessDenied, AccessError, UserError, ValidationError
from eagle.http import request
from eagle.osv import expression
from eagle.service.db import check_super
from eagle.tools import partition, collections, lazy_property
...
class Users(models.Model):
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Eagle (updates, module installation, ...)'))
        db = self._cr.dbname
        for id in self.ids:
            self.__uid_cache[db].pop(id, None)
        self._invalidate_session_cache()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: eagle.exceptions.AccessDenied when old password is wrong
        :raise: eagle.exceptions.UserError when new password is not set or empty
        """
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warn(
                    ...
                    "and *might* be a proxy. If your Eagle is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Eagle in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.eagle.com/documentation/13.0/setup/deploy.html#https for details.",
                    source
                )
