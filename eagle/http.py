...
# *broken*
NO_POSTMORTEM = (eagle.exceptions.except_orm,
                 eagle.exceptions.AccessDenied,
                 eagle.exceptions.Warning,
                 eagle.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                eagle.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = eagle.service.common.dispatch
        elif service_name == 'db':
            dispatch = eagle.service.db.dispatch
        elif service_name == 'object':
            dispatch = eagle.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                eagle.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                eagle.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except eagle.exceptions.DeferredException as e:
        _logger.exception(eagle.tools.exception_to_unicode(e))
        eagle.tools.debugger.post_mortem(eagle.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(eagle.tools.exception_to_unicode(e))
        eagle.tools.debugger.post_mortem(eagle.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    ...
    @property
    def cr(self):
        """ :class:`~eagle.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~eagle.api.Environment` bound to current request. """
        if self._env is None:
            self._env = eagle.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            eagle.tools.debugger.post_mortem(
                eagle.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <eagle.http.WebRequest.csrf_token`, the
          :data:`~eagle.http.request` object is available by default
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (eagle.exceptions.Warning, eagle.exceptions.except_orm,
...
def serialize_exception(e):
    ...
    elif isinstance(e, eagle.exceptions.Warning):
    ...
    elif isinstance(e, eagle.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, eagle.exceptions.except_orm):
...
HTTP methods). See
https://www.eagle.com/documentation/13.0/reference/http.html#csrf for
more details.
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["eagle", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('eagle.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(werkzeug.contrib.sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None, uid=None):
        ...
        if uid is None:
            ...
            uid = eagle.registry(db)['res.users'].authenticate(db, login, password, env)
        else:
    ...
    def check_security(self):
        ...
        env = eagle.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or eagle.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = eagle.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint, routing in eagle.http._generate_routing_rules([''] + eagle.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in eagle.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        eagle.registry(db).check_signaling()
                        with eagle.tools.mute_logger('eagle.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = eagle.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if eagle.tools.config['dbfilter']:
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = eagle.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = eagle.tools.ustr(filename)
